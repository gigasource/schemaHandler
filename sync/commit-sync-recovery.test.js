...
describe("commit-sync", function () {
  ...
  it("case 1", async function (done) {
    ...
    const m1 = await ormA('Model').create({table: 10, items: []}).commit("create", {
      table: 10
    });

    //await delay(200);
    //await callbackLock.acquireAsync();
    const m2 = await ormA('Model').findOneAndUpdate({_id: m1._id}, {$push: {items: 'item1'}}).commit("addItem", {
      table: 10
    });
    ...
    const m4 = await ormA('Model').updateOne({table: 10}, {$push: {items: 'item2'}}).commit("addItem", {
      table: 10
    });
