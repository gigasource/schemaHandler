...
describe("commit-sync", function() {
  beforeAll(async () => {
    ...
    ormB.on("createCommit:master", async function(commit) {
      if (commit.tags.includes("create")) {
        const activeOrder = await ormB(`${commit.collectionName}`).findOne({
          table: commit.data.table
        });
        if (activeOrder) {
          //create doNothing Commit here
          commit.id = (await ormB.emit("getHighestCommitId")).value + 1;
          commit.approved = false;
          delete commit.chain;

          this.value = await ormB(`Commit`).create(commit);
          return;
        }
      }
      await ormB.emitDefault("createCommit:master", commit);
    });
